...
function 2_Up {
    ...
    $json = '{"targets":[{"target":"url", "constraint":{"operator":"matches","value":"$(dnsName)/*"}}],"actions":[{"id":"forwarding_url","value": {"url": "https://$(dnsName)/$1","status_code": 301}}],"priority":1,"status":"active"}'
    $headers = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
    $headers.Add("X-Auth-Key", $cloudFlareKey)
    $headers.Add("X-Auth-Email", $cloudFlareEmail)
    $headers.Add("Content-Type", "application/json")
